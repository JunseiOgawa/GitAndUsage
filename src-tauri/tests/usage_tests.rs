...
fn test_save_and_load_normal_snapshots() {
    ...
    assert!(save_result.is_ok(), "Saving normal snapshots failed: {:?}", save_result);
...
fn test_save_snapshots_creates_parent_directories() {
    ...
    let file_path = dir.path().join("nested_dir").join("deeply_nested").join("snapshots.json");
    ...
    let save_result = save_snapshots_to_json(&file_path, &snapshots);
    assert!(save_result.is_ok(), "Saving and creating directories failed: {:?}", save_result);
...
fn test_load_non_existent_file() {
    ...
    assert!(loaded.is_empty(), "Loading non-existent file should return an empty Vec, got: {:?}", loaded);
}
...
fn test_load_directory_as_file() {
    ...
    assert!(loaded.is_empty(), "Loading a directory as a file should gracefully return an empty Vec, got: {:?}", loaded);
}
...
fn test_load_empty_file() {
    ...
    let loaded = load_cached_snapshots(&file_path);
    assert!(loaded.is_empty(), "Loading an empty file should return an empty Vec, got: {:?}", loaded);
}
...
fn test_load_malformed_json_garbage() {
    ...
    fs::write(&file_path, "not a valid json string, just garbage text!!!").expect("Failed to write garbage file");

    let loaded = load_cached_snapshots(&file_path);
    assert!(loaded.is_empty(), "Loading random garbage JSON should return an empty Vec fallback, got: {:?}", loaded);
}
...
fn test_load_malformed_json_object() {
    ...
    fs::write(&file_path, r#"{"provider": "codex", "display_name": "Codex"}"#).expect("Failed to write object JSON file");

    let loaded = load_cached_snapshots(&file_path);
    assert!(loaded.is_empty(), "Loading valid JSON object (expected array) should return an empty Vec, got: {:?}", loaded);
}
...
fn test_load_malformed_json_missing_fields() {
    ...
    let loaded = load_cached_snapshots(&file_path);
    assert!(loaded.is_empty(), "Loading invalid schema array should return an empty Vec, got: {:?}", loaded);
}
...
fn test_save_invalid_path_file_in_way_errors() {
    ...
    assert!(save_result.is_err(), "Saving to a path blocked by a file should fail and return Err");
}
...
fn test_save_invalid_path_null_byte_errors() {
    ...
    assert!(save_result.is_err(), "Saving to a path with null bytes should return Err");
}
